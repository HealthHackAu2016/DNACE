...
var Player = SheetSprite.extend({
  ...
  update: function(dt) {
    ...
    //this.position.x += this.xVel - 50;
	this.position.x += direction*50; //step of 50
	if (this.position.x < 50) {
		direction = direction*-1;
	} else if (this.position.x > (CANVAS_WIDTH - 50)) {
		direction = direction*-1;
	}
...
var Enemy = SheetSprite.extend({
  ...
  update: function(dt) {
    ...
    if (this.stepAccumulator >= this.stepDelay) {
      ...
      this.position.x += 0 * alienDirection;
      this.toggleFrame();
      this.stepAccumulator = 0;
    }
...
function drawStartScreen() {
  fillCenteredText("DNACE!!!", CANVAS_WIDTH/2, CANVAS_HEIGHT/2.75, '#FFFFFF', 36);
